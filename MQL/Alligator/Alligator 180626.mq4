               ...
               if (objAlg.getDirec() == cintDirec) { //is the new time frame direction the same?
                  if (objAlg.signalGet()) { //has a signaled appeared in the new time frame?
                     if (objAlg.signalOvercome()) { //can trade in the new time frame, i.g. signal triggered?
                        datetime dtmSignalTimePrior = cobjAlg.getSignalTime(); //remember the prior signal to use it the shifted time frame //TO DO: trend_child/parent will contain the prior direction
                        ...
                        cobjAlg.setSignalTime(dtmSignalTimePrior); //restore the original signal generated in the prior time frame //TO DO: trend_child/parent will contain the prior and new time frame signals; it's more robust to remember the new time frame signal in a way
...
void delay() { //+------------------------------------------------------------------+
   long j = 0;
   long lngDelay = (int)GlobalVariableGet(mstrDelay);
   if (Time[mintIdx0] >= StringToTime(TimeDelay)) 
      lngDelay = (long)GlobalVariableGet(mstrDelayLong);
...
void OnTick() { //+------------------------------------------------------------------+
   delay();
   if (CheckPointer(mobjTrend) == POINTER_INVALID) 
      mobjTrend = new trend(Bid); //a new trend began; create an object for it, unless available //TO DO: unite the three events (trend + signal + trade): create a trend object on the first trade within the trend
         ...
         if (delete_old_trend()) OnTick(); //the old trend has become obsolete; restart the procedure
            ...
            if (mobjTrend.alg_SignalGet()) {
               if (!mobjTrend.alg_SignalTeethFarGetSave(SignalTeethFarness)) { //Isn't the signal too far from the teeth?
                  mobjTrend.setPhase(phase_signal_appeared); //TO DO: incapsulate flagging this cintPhase into the trend class; currently this cintPhase can be switched either insdie alligator or outside trend
                  Alert("Alligator signal_appeared!");
                  mobjTrend.trade_open();
         }  }  }
         break; //no signal yet? //TO DO: unite the three events (trend + signal + trade): create a trend object on the first trade within the trend
      case phase_signal_appeared:
         if (delete_old_trend()) OnTick(); //Has not the old trend become obsolete? Restart the procedure //TO DO: unite the three events (trend + signal + trade): create a trend object on the first trade within the trend
   ...
   if (CheckPointer(mobjTrend) == POINTER_DYNAMIC) mobjTrend.update_peak(Bid); //update the peak for a later use, if a trend exists. //TO DO: why not placing it at the top below mobjTrend.cobjAlg.getSave(mintIdx0)?
}
