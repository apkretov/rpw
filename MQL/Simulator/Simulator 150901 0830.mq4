...
void delay() { //+------------------------------------------------------------------+
   ...
   if (ObjectFind(strSuspend) == 0) {
      if (Time[0] >= ObjectGet(strSuspend, OBJPROP_TIME1)) lngDelay = (long) GlobalVariableGet(mstrDelayLong);
   } else if (Time[0] >= MathMin(StringToTime(TimeDelay), GlobalVariableGet(mstrTimeDelay))) lngDelay = (long) GlobalVariableGet(mstrDelayLong);
