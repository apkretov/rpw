...
void terminal::getOfferBidBest(int intFrom, int intTo, bool(*fncInDecrmentCompare)(int&, int), double& dblPrice, int& intQuantity) { // Get the best bid/offer. 
	while (fncInDecrmentCompare(intFrom, intTo)) { //The best bid is placed at the top of the bids DOM, the best offer is at the botteom.
		...
		if (lua_isnil(L, -1)) lua_pop(L, 2); //На некоторых ценах могут отсутствовать заявки //Clear the stack off Price/Quantity record number intFrom and the nil quantity.
		else {
...
bool terminal::getOfferBid(const string& strFutures) { 
		...
		lua_getfield(L, -1, "bid_count"); intBidCount = lua_tointeger(L, -1); lua_pop(L, 1); //Количество котировок покупки. При отсутствии спроса возвращается значение «0» //getQuoteLevel2 @ C:\Program Files (x86)\Lua\5.1\docs\lua5_1_4_Docs.chm
		lua_getfield(L, -1, "offer_count"); intOfferCount = lua_tointeger(L, -1); lua_pop(L, 1); //Количество котировок продажи. При отсутствии предложения возвращается значение «0» //getQuoteLevel2 @ C:\Program Files (x86)\Lua\5.1\docs\lua5_1_4_Docs.chm
...
void terminal::createOperationsTable(int (*tableNotificationCallback)(lua_State*)) { 
	...
	constexpr int QTABLE_STRING_TYPE = 3; //par_type – тип данных в колонке: строка. Because it isn't documented, I printed the constant's value from MT4QuikConnector_.lua. //QLUA.chm\AddColumn.
	constexpr int QTABLE_DEFAULT_COLOR = -1; //Если в качестве цвета задана константа QTABLE_DEFAULT_COLOR, то используется цвет, заданный в цветовой схеме операционной системе Windows. //Because it isn't documented, I printed the constant's value from MT4QuikConnector_.lua. //SetColor @ QLUA.chm.
	lua_getglobal(L, "AllocTable"); lua_pcall(L, 0, 1, 0); cintOperationsTable = lua_tointeger(L, -1); lua_pop(L, 1);
	lua_getglobal(L, "AddColumn"); lua_pushinteger(L, cintOperationsTable); lua_pushinteger(L, BUYtoSELL(OP_SELL)); lua_pushstring(L, "SELL"); lua_pushboolean(L, true); lua_pushinteger(L, QTABLE_STRING_TYPE); lua_pushinteger(L, intColumnWidht); lua_pcall(L, 6, 1, 0); lua_pop(L, 1); //Функция возвращает «1», если колонка в таблицу добавлена, иначе – «0». //AddColumn @ QLUA.chm.
	lua_getglobal(L, "AddColumn"); lua_pushinteger(L, cintOperationsTable); lua_pushinteger(L, BUYtoSELL(OP_BUY)); lua_pushstring(L, "BUY"); lua_pushboolean(L, true); lua_pushinteger(L, QTABLE_STRING_TYPE); lua_pushinteger(L, intColumnWidht); lua_pcall(L, 6, 1, 0); lua_pop(L, 1); //Функция возвращает «1», если колонка в таблицу добавлена, иначе – «0». //AddColumn @ QLUA.chm.
	lua_getglobal(L, "CreateWindow"); lua_pushinteger(L, cintOperationsTable); lua_pcall(L, 1, 1, 0); lua_pop(L, 1); //Функция возвращает «1» при успешном создании окна, иначе – «0». //CreateWindow @ QLUA.chm.
	...
	lua_getglobal(L, "SetWindowCaption"); lua_pushinteger(L, cintOperationsTable), lua_pushstring(L, "5 SELL BUY"); lua_pcall(L, 2, 1, 0); lua_pop(L, 1); //В случае успешного выполнения возвращает «true», иначе – «false». Clear the stack off it. //SetWindowCaption @ QLUA.chm.
	lua_getglobal(L, "SetWindowPos"); lua_pushinteger(L, cintOperationsTable); lua_pushinteger(L, intPosX); lua_pushinteger(L, intPosY); lua_pushinteger(L, intPosDX); lua_pushinteger(L, intPosDY); lua_pcall(L, 5, 1, 0); lua_pop(L, 1); //В случае успешного выполнения возвращает «true», иначе – «false». 
	lua_getglobal(L, "SetTableNotificationCallback"); lua_pushinteger(L, cintOperationsTable); lua_pushcfunction(L, tableNotificationCallback); lua_pcall(L, 2, 1, 0); lua_pop(L, 1); //В случае успешного завершения функция возвращает «1», иначе – «0». 
}
...
void terminal::message_(const string& strMessage, int intIcon_type) const { //выводит сообщение в терминале //пример реализации функции обратного вызова onalltrade внутри dll @ http://quikluacsharp.ru/qlua-c-cpp-csharp/primer-realizatsii-funktsii-obratnogo-vyzova-onalltrade-vnutri-dll/ 
