...
int candlesByIndex::iBars() const { 
	
	//Qlua: getNumCandles("AlgSR").
	lua_getglobal(L, "getNumCandles"); 
	lua_pushstring(L, cstrTag.c_str()); 
	lua_pcall(L, 1, 1, 0); 
	int intBars = lua_tointeger(L, -1); 
	lua_pop(L, 1); 
...
void candlesByIndex::setCandlesByIndex() { // Set the object. 
	...
	//¬озвращаемые значени€ : t Ц таблица, содержаща€ запрашиваемые свечки, n Ц количество свечек в таблице t, l Ц легенда(подпись) графика.
	lua_getglobal(L, "getCandlesByIndex"); 
	lua_pushstring(L, cstrTag.c_str()); 
	lua_pushinteger(L, cintLine); 
	lua_pushinteger(L, cintBars - cintOffset - cintCount); 
	lua_pushinteger(L, cintCount); 
	lua_pcall(L, 4, 3, 0); 
	
	cintN = lua_tointeger(L, -2); lua_pop(L, 2); //Clear the stack off n and l.
	
	//Save the t table, the 1st result getCandlesByIndex returns. 
	//Pop a value from the stack, stores it into the registry with a fresh integer key, and returns that key. 27.3.2 Ц References @ http://www.lua.org/pil/27.3.2.html
	cintT = luaL_ref(L, LUA_REGISTRYINDEX);
}

// Release both the value and the reference. 27.3.2 Ц References @ http://www.lua.org/pil/27.3.2.html 
void candlesByIndex::deleteCandlesByIndex() { 
	luaL_unref(L, LUA_REGISTRYINDEX, cintT); 
} 

// Refresh the object. 
void candlesByIndex::refresh() { 
...
double candlesByIndex::getOpen(int intShift) const { 
	return OHLC("open", intShift); 
}

double candlesByIndex::getHigh(int intShift) const { 
	return OHLC("high", intShift); 
}

double candlesByIndex::getLow(int intShift) const { 
	return OHLC("low", intShift); 
}

double candlesByIndex::getClose(int intShift) const { 
	return OHLC("close", intShift); 
}
...
datetime candlesByIndex::getDatetime(int intShift) const {
	...
	lua_getfield(L, intTopDatetime, "mcs"); 
	int intMcs = lua_tointeger(L, -1);
	
	lua_getfield(L, intTopDatetime, "ms"); 
	int intMs = lua_tointeger(L, -1);
	
	lua_getfield(L, intTopDatetime, "sec"); 
	int intSec = lua_tointeger(L, -1);
	
	lua_getfield(L, intTopDatetime, "min"); 
	int intMin = lua_tointeger(L, -1);
	
	lua_getfield(L, intTopDatetime, "hour"); 
	int intHour = lua_tointeger(L, -1);
	
	lua_getfield(L, intTopDatetime, "day"); 
	int intDay = lua_tointeger(L, -1);
	
	lua_getfield(L, intTopDatetime, "month"); 
	int intMonth = lua_tointeger(L, -1);
	
	lua_getfield(L, intTopDatetime, "year"); 
	int intYear = lua_tointeger(L, -1);
	
	//Clear the stack off the t table, the record in the t table, the datatime table, and all the datetime values (from intTopDatetime atop).
	lua_pop(L, lua_gettop(L) - intTop - intTopDatetime); 
