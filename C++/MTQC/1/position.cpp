...
bool position::sendTransaction_(int intOperation, const terminal& objTerminal, const string& strOpenClose, int intLots) { 
	static int intStockFuturesDiff = /*500*/1000; //The difference between the prices of the stock and futures to order the worst futures price w/o the DOM.
	bool blnSendTransaction{}; //Get to Know the New C++11 Initialization Forms @ http://www.informit.com/articles/article.aspx?p=1852519 //Introducing C++11 Brace-Initialization //string s{}; //same as: string s; @ http://www.informit.com/articles/article.aspx?p=1852519
...
#else // If no tape (OnAllTrade in MT4QuikConnector.cpp is not invoked) set the stock price manually.
	double dblPrice = gdblPriceStock
		* gintContractSize 
		+ (intOperation == OP_BUY ? intStockFuturesDiff : -intStockFuturesDiff); 
#endif
...
messages::message("111 sendTransaction_: PRICE: " + strPriceFractional);
	...
	if (cblnTradeDisabled) { // ["flags"]: бит 2 (0x4)  Заявка на продажу, иначе – на покупку. Данный флаг для сделок и сделок для исполнения определяет направление сделки (BUY/SELL) 	//@ QLUA.chm\Флаги для таблиц Заявки, Заявки на внебиржевые сделки, Сделки, Сделки для исполнения
...
return true;/*1*/
		...
		if (strSendTransaction != "") // ЕСЛИ функция вернула строку диагностики ошибки, ТО значит транзакция не прошла // Выводит сообщение с ошибкой @ http://quikluacsharp.ru/quik-qlua/primer-skripta-qlua-lua-vypolnyayushhego-torgovye-operatsii-v-terminale-quik/
