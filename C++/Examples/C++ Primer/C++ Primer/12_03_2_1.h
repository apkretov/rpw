...
private:
	...
	map<string, shared_ptr<set<line_no>>> wm; //TEST //																																			map of each word to the set of the lines in which that word appears
};
...
TextQuery::TextQuery(ifstream &is) : file(new vector<string>) { // read the input file and build the map of lines to line numbers
	...
	while (getline(is, text)) { // for each line in the file
		...
		while (line >> word) { // for each word in that line
			auto &lines = wm[word]; //TEST!! //																																					if word isn’t already in wm, subscripting adds a new entry // lines is a shared_ptr
			if (!lines) //TEST! //																																								that pointer is null the first time we see word
				lines.reset(new set<line_no>); //TEST!!! //																																allocate a new set
