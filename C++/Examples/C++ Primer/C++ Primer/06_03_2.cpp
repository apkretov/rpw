...
namespace How_Values_Are_Returned {
	string make_plural(size_t ctr, const string &word, const string &ending) { // return the plural version of word if ctr is greater than 1
		return (ctr > 1) ? word + ending : word;
	}

	const string &shorterString(const string &s1, const string &s2) { //TEST // return a reference to the shorter of two strings
...
namespace Never_Return_a_Reference_or_Pointer_to_a_Local_Object {
	...
	const string &manip() { //																																											disaster: this function returns a reference to a local object
		...
		if (!ret.empty()) // transform ret in some way
			return ret; //TEST!! //																																										WRONG: returning a reference to a local object!
		else
			return "Empty"; //TEST!! //																																									WRONG: "Empty" is a local temporary string
	}

#elif ON
	//TEST! const int &mine1() {  
		int i{111};
		return i;
	}

#elif ON
	//TEST! const int *mine2() { 
		int i{222};
		return &i;
	}

#elif ON
	//TEST! const string &mine3() {
		string s{"aaa"};
		return s;
	}

#elif ON
	//TEST! const string *mine4() {
		string s{"bbb"};
		return &s;
	}
#endif
...
namespace Reference_Returns_Are_Lvalues { //TEST
...
int main(int argc, char *argv[]) {
	...
	cout << How_Values_Are_Returned::make_plural(2, "word", "s") << '\n'; //MINE
...
int &get(int *arry, int index) { //TEST
	return arry[index]; 
}
