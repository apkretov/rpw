...
public:
	...
	Blob(); // constructors
	explicit Blob(initializer_list<T> il); //MINE //ORIG Blob(initializer_list<T> il);
...
template <typename T>
Blob<T>::Blob() try : data(make_shared<vector<T>>()) {/*empty*/}
catch (const bad_alloc &e) {
	handle_out_of_memory(e);
}
#pragma endregion
...
#pragma region EX_18_07
Blob<T>::Blob(It b, It e) try : data(make_shared<vector<T>>(b, e)) {/*empty*/}
catch (const bad_alloc &e) {
	handle_out_of_memory(e);
}
#pragma endregion
