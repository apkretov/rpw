...
template <typename T> 
class Vec { //VER1 // Based on class StrVec { //VER5 
public:
	Vec() = default;
...
private:
	...
	pair<T *, T *> alloc_n_copy(const T *, const T *) const;
...
void Vec<T>::push_back(T &&s) {
	cout << "push_back(T &&)\n"; //MINE
	chk_n_alloc(); // ensure that there is room for another element
	std::construct_at(first_free++, std::move(s));
}
...
int main(int argc, char *argv[]) {
	...
	vi.push_back(2);
	vi.push_back(3);
	std::ranges::copy(vi, ostream_iterator<int>(cout, " "));

	cout << "\n\n";
	Vec vd{1.1, 2.2, 3.3};
	std::ranges::copy(vd, ostream_iterator<double>(cout, " "));

	cout << "\n\n";
	Vec<string> vs{"one", "two", "three"};
	std::ranges::copy(vs, ostream_iterator<string>(cout, " "));
	cout << '\n';
