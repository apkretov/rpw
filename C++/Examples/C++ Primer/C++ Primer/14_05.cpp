...
void print_StrVec(const StrVec &s) { //MINE
...
int main(int argc, char *argv[]) {
	...
	if (svec.size() && svec[0].empty()) { // if svec has any elements, run the string empty function on the first one
		svec[0] = "zero"; // ok: subscript returns a reference to a string
		//TEST cvec[0] = "Zip"; // error: subscripting cvec returns a reference to const
	}
	print_StrVec(svec);
#endif
