...
public:
    GumballMachineProxy(boost::asio::io_context& io_context, const std::string& host, unsigned short port)
	...
	struct MachineInfo {		// Maintain the connection between requests:
        std::string location;	// 1. Make a single network request to fetch all machine information at once
        int count;				// 2. Cache the results
        std::string state;		// 3. Return cached values for subsequent calls
