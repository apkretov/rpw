...
class Flock : public Quackable { // Remember, the composite needs to implement the same interface as the leaf elements. Our leaf elements are Quackables.
	std::vector<QuackablePtr> quackers; // We're using an ArrayList inside each Flock to hold the Quackables that belong to the Flock.
public:
	void add(QuackablePtr quacker) { quackers.push_back(std::move(quacker)); } // The add() method adds a Quackable to the Flock.

	void quack() override {				// Now for the quack() method - after all, the Flock is a Quackable too. The quack() method in Flock needs
		for (auto &quacker : quackers)	// to work over the entire Flock. Here we iterate through the ArrayList and call quack() on each element.
			quacker->quack();
	}
};
