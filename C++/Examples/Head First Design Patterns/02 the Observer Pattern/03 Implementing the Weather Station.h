...
public:
	...
	void display() override { std::print("Current conditions: {:.1f}F degrees and {:.1f}% humidity\n", temperature, humidity); } // The display() method just prints out the most recent temp and humidity.
};
...
class StatisticsDisplay : public Observer, public DisplayElement { // This display implements Observer so it can get changes from the WeatherData object. // It also implements DisplayElement, because our API is going to require all display elements to implement this interface.
	std::vector<double> temp_measurs;
	double avg_temp = 0;
	double min_temp = 0;
	double max_temp = 0;
	Subject &weatherData_;
public:
	...
	void display() override { std::print("Avg/Max/Min temperature = {:.1f}/{:.1f}/{:.1f}\n", avg_temp, max_temp, min_temp); }
};
...
public:
	...
	void update(double temperature, double humidity, double pressure) override {
		...
		this->humidity = std::min(humidity + 10.0, 100.0);
		display();
	}

	void display() override { std::print("Forecast: {:.1f}F degrees and {:.1f}% humidity\n", temperature, humidity); } 
};
