...
int main() { // This is our Client in Command Pattern-speak.
	...
	LightOnCommand lightOn(light); // Here, create a command and pass the Receiver to it.
	remote.setCommand(make_unique<LightOnCommand>(lightOn)); // Here, pass the command to the Invoker.
	remote.buttonWasPressed(); // And then we simulate the button being pressed.
