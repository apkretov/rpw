...
class TurkeyAdapter : public Duck { // First, you need to implement the interface of the type you're adapting to.This is the interface your client expects to see.
    Turkey& turkey;
public:
    explicit TurkeyAdapter(Turkey& turkey) : turkey(turkey) {} // Next, we need to get a reference to the object that we are adapting; here we do that through the constructor.
