...
int main() {
	...
	cout << "--- Situation 1: Initialization with std::call_once --- ";
	...
	for (int i = 0; i < 3; ++i) 
		threads.emplace_back(use_x_in_thread, ref(x_threaded));
	for (auto& t : threads) 
		t.join();
	cout << "All threads finished. open_connection() was called only once.\n";

#ifdef SITUATION_2
	cout << "\n--- Situation 2: Handling non-copyable std::once_flag --- ";

	cout << "Class X with a std::once_flag member is not copyable by default.\n";
	// X x2 = x; // This line would cause a compilation error.

	copyable_X cx1(details);
	cout << "Created cx1. Initializing its connection.\n";
	cx1.send_data(data_packet{});

	cout << "\nCreating cx2 as a copy of cx1.\n";
	copyable_X cx2 = cx1; // Calls the user-defined copy constructor.

	cout << "cx2 is a new object with its own uninitialized connection.\n";
	cout << "Initializing connection for cx2.\n";
