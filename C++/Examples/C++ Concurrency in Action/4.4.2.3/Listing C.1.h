...
struct wrapped_message : message_base { // Each message type has a specialization.
	Msg contents;
...
class queue { // Your message queue
	...
	std::queue<std::shared_ptr<message_base>> q; // Internal queue stores pointers to message_base
public:
	template<typename T>
