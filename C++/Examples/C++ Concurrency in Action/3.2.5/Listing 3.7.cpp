...
void thread_a() {
	try {
		...
		std::cerr << std::this_thread::get_id() << " Thread A exception: " << e.what() << '\n';
	}
}
...
void thread_b() {
	try {
		...
		std::cerr << std::this_thread::get_id() << " Thread B exception: " << e.what() << '\n';
	}
}
...
int do_low_level_stuff() {
	std::cout << std::this_thread::get_id() << " Low level work done.\n";
	return 42;
}

void high_level_stuff(int some_param) { std::cout << std::this_thread::get_id() << " High level work with param: " << some_param << "\n"; }

void do_other_stuff() { std::cout << std::this_thread::get_id() << " Doing other stuff.\n"; }
