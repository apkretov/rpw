#if 0 //@ https://github.com/Hrodvintir/-basics-of-c-plus-plus-development-brown-belt/tree/main/week1/04%20Practise%20Assingment
...
class Hasher {
	hash<CoordType> ct_hash;
public:
	size_t operator()(const Point3D &p) const {
		constexpr int x =
#ifdef MINE
		37;
#else //GITHUB @ https://github.com/Hrodvintir/-basics-of-c-plus-plus-development-brown-belt/blob/main/week1/04%20Practise%20Assingment/solution.cpp
		2'946'901; // выбираем в качестве коэффициента довольно большое простое число; свободный член можем положить равным нулю, т.к. он не влияет на коллизии, а лишь циклически смещает бакеты
#endif //GITHUB
		return (ct_hash(p.x) * x * x) + ct_hash(p.y) * x + ct_hash(p.z); // Ax^2 + Bx + C
	}
};
...
В своей реализации используйте стандартную хеш-функцию std::hash для хеширования координат и комбинируйте полученные хеши с помощью полинома, как показано в лекции.
