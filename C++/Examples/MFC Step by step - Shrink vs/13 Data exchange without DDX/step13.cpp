...
public:
   afx_msg void OnLButtonDblClk(UINT, CPoint) { MoveWindow(CRect(120, 100, 220, 150), TRUE); };
   afx_msg void OnRButtonDblClk(UINT, CPoint) { MoveWindow(CRect(120, 10, 220, 50), TRUE); }; 
};
...
public:
	CMyPaintDC(CWnd* pWnd) : CPaintDC(pWnd) { }; // мы дети CPainDC
	void DrawCross(int iPosX, int iPosY) {	// функция рисования креста // для создания своей фигуры используем функции CPaintDC 
...
public:
	CMyDialog(UINT nIDTemplate, CWnd* pParentWnd = NULL) : CDialog(nIDTemplate, pParentWnd) { cListBox = NULL; }; // Объекта нет, NULL
	~CMyDialog() { if (cListBox !=  NULL) delete cListBox; }; // если объект был создан удалить
	CString GetString(){	return csListTextItem; }; // вернуть результат
	...
	virtual void OnOK() { // когда нажмут кнопку OK
...
CMainWnd::CMainWnd() {
	...
	Create(NULL, _T("Step13"), WS_OVERLAPPEDWINDOW, rectDefault, NULL, NULL); // Создать окно программы
