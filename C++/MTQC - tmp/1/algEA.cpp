...
namespace mtqc {
	...
	namespace algEA {
		...
		/*inline*/ double distance(double dblFrom, double dblTo, enmDirec intDirec) { 
			return (dblTo - dblFrom) * (int)intDirec; 
		}

		int distance_pts(double dblFrom, double dblTo, enmDirec intDirec) { 
			return roundToPts((dblTo - dblFrom) * (int)intDirec); 
		}
					...
					if (delete_old_trend()) OnTickAlg(); //the old trend has become obsolete; restart the procedure
						...
						if (gobjTrend->alg_SignalGet()) {
							if (!gobjTrend->alg_SignalTeethFarGetSave(SignalTeethFarness)) { //Isn't the signal too far from the teeth?
								gobjTrend->setPhase(enmPhases::phase_signal_appeared); //develop: incapsulate flagging this phase into the trend class; currently this phase can be switched either insdie alg or outside trend
