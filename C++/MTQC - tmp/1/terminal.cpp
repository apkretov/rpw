...
bool terminal::getOfferBid(const string& strFutures) { //TO DO: Use just the traded tools table with the best bid/offer instead of getQuoteLevel2.
	...
	if (lua_pcall(L, 2, 1, 0) != EXIT_SUCCESS || !lua_istable(L, -1)) //Call the getQuoteLevel2 QLua function. Did it succeed?
		return false; 
		
	int intBidCount = 0, intOfferCount = 0; //KTB DEBUGGING L = luaL_newstate(); luaL_dofile(L, "Data.lua"); lua_getglobal(L, "ql2"); 
		
	lua_getfield(L, -1, "bid_count"); //Количество котировок покупки. При отсутствии спроса возвращается значение «0» getQuoteLevel2 @ C:\Program Files (x86)\Lua\5.1\docs\lua5_1_4_Docs.chm
	intBidCount = lua_tointeger(L, -1); 
	lua_pop(L, 1); 
		
	lua_getfield(L, -1, "offer_count"); //Количество котировок продажи. При отсутствии предложения возвращается значение «0» getQuoteLevel2 @ C:\Program Files (x86)\Lua\5.1\docs\lua5_1_4_Docs.chm
	...
	return cdblOfferBidPrice[OP_BUY] //Is there a bid and an offer?
...
void terminal::createOperationsTable(int (*tableNotificationCallback)(lua_State*)) { 
	...
	//Adjacent to volume.
	constexpr int intPosX = /*ORIG 456*//*800*//*850*//*880*//*895*//*902*//*905*//*908*//*909*//*910*/911, intPosY = /*ORIG 318*//*310*//*311*/312; 
	constexpr int intPosDX = /*ORIG 160*//*220*//*234*/235/*250*/, intPosDY = /*ORIG 74*//*85*//*90*/91/*92*//*100*/;
