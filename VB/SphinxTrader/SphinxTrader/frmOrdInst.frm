...
On Error GoTo ErrHnd
   Const strTime As String = "TIME" 'DDE link topic settings.
   ...
   Call LinkPrices(lblTime, mintAutomatic, dbcTools.Text, strTime) ' Link the DDE lable.
   With adoTools.Recordset
      .MoveFirst
      .Find (dbcTools.ListField & " = '" & dbcTools.BoundText & "'")
      If .BOF = False And .EOF = False Then lngFractMult = adoTools.Recordset.Fields("FractMult")
   End With
   gcnnConnServ.Execute "Ins_Trades(" & gintIDAcc & ", '" & lblTime & "', " & intTyp & ", " & ConvCur(txtVol) * intFractMultVol & ", '" & CInt(dbcTools.BoundText) & "', " & lngFractMult & ", " & ConvCur(strBidAsk) * lngFractMult & ", " & ConvCur(txtSL) * lngFractMult & ", " & ConvCur(txtTP) * lngFractMult & ", '" & datNull & "', 0, 0, 0)", , adCmdStoredProc + adExecuteNoRecords
Exit Sub
ErrHnd:
MsgBox Err.Number & vbTab & Err.Description
End Sub
...
On Error GoTo ErrHnd
   Const intNotify As Integer = 3 'Notify link mode
   Const strBid As String = "BID" 'DDE link topic settings.
   Const strAsk As String = "ASK"
   Const strTime As String = "TIME" 'DDE link topic settings.
   Call LinkPrices(lblBid, mintAutomatic, dbcTools.Text, strBid) ' Link the DDE price lables.
   Call LinkPrices(lblAsk, mintAutomatic, dbcTools.Text, strAsk)
   Call LinkPrices(lblNotify, intNotify, dbcTools.Text, strTime)
...
Private Sub LinkPrices(lblPrice As Label, ByVal intLinkMode As Integer, ByVal strTool As String, ByVal strBidAsk As String)
On Error GoTo ErrHnd
   ...
   lblPrice.LinkTopic = strDDEApp & "|" & strBidAsk 'Sets up link with VB source.
   lblPrice.LinkItem = strTool 'Set link to text box on source.
   lblPrice.LinkMode = intLinkMode 'Reestablish new LinkMode.
Exit Sub
ErrHnd:
    If Err.Number <> intErrNoDDE Then 'An error other than Foreign application won't perform DDE method or operation?
        MsgBox Err.Number & vbTab & Err.Description
    End If
End Sub
...
Private Sub SetStpLmt(cmdSellBuy As CommandButton, intTypPend As Integer, intTypPendAbv As Integer, intTypPendBlw As Integer, ByVal strBidAsk As String, ByVal strSellBuy As String, ByVal strStpLmtAbv As String, ByVal strStpLmtBlw As String)
   Dim curBidAsk As Currency
   curBidAsk = CDec(ConvCur(strBidAsk)) ' Change the number locale decimal symbol.
...
'   mintTypPendBuy = 0 'Reset the pending Buy/Sell Stop/Limit order flags.
'   mintTypPendSell = 0
'   Call ResetBuySellStpLmt ' Reset the pending Buy/Sell Stop/Limit order flags.
End Sub

'' Reset the pending Buy/Sell Stop/Limit order flags.
'Private Sub ResetBuySellStpLmt()
'   mintTypPendBuy = 0
'   mintTypPendSell = 0
'End Sub
...
On Error GoTo ErrHnd
   ...
   Set dbcTools.RowSource = adoTools
   dbcTools.BoundColumn = "ID"
   dbcTools.ListField = "Tool"
Exit Sub
ErrHnd:
MsgBox Err.Number & vbTab & Err.Description
End Sub
...
Private Sub lblNotify_LinkNotify()
   Debug.Print lblBid & vbTab & lblAsk
End Sub
