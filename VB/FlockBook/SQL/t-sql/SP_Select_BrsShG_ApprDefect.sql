USE FlockBook
GO
IF EXISTS (SELECT name FROM sysobjects WHERE NAME = "spSelect_BrsShG_ApprDefect" AND type = "P")
    DROP PROCEDURE spSelect_BrsShG_ApprDefect
GO
CREATE PROCEDURE spSelect_BrsShG_ApprDefect (@IDFarm char(2), @DateDispMin int, @DateDispMax int) AS
--    SET DATEFORMAT dmy
	...
	WHERE (SheepGoat.IDFarm = @IDFarm) AND 
	    ...
	    (InhertDefect.IDDefect IS NOT NULL)
GO
spSelect_BrsShG_ApprDefect "ÀÀ", 0, 65500 -- All.
--spSelect_BrsShG_ApprDefect "ÀÀ", 0, 0 -- Kept.
--spSelect_BrsShG_ApprDefect "ÀÀ", 1, 65500 -- Disposed.
GO