...
Begin VB.Form frmShG 
   ...
   Begin VB.Frame fraFrame 
      ...
      Begin MSHierarchicalFlexGridLib.MSHFlexGrid flexGrid 
         ...
         FocusRect       =   2
         AllowUserResizing=   3
         DataMember      =   "spSelect_Mate_IDDam"
         _NumberOfBands  =   3
         _Band(0).Cols   =   3
   ...
   Begin VB.Frame fraFrame 
      ...
      Begin MSHierarchicalFlexGridLib.MSHFlexGrid flexGrid 
         ...
         FocusRect       =   2
         AllowUserResizing=   3
         DataMember      =   "spSelect_WeightWoolAnnl_IDShG"
         _NumberOfBands  =   1
         _Band(0).Cols   =   5
         _Band(0).GridLinesBand=   1
         _Band(0).TextStyleBand=   0
   ...
   Begin VB.Frame fraFrame 
      ...
      Begin MSHierarchicalFlexGridLib.MSHFlexGrid flexGrid 
         ...
         FocusRect       =   2
         AllowUserResizing=   3
         DataMember      =   "spSelect_Appraisal_IDShG"
         _NumberOfBands  =   2
         _Band(0).Cols   =   18
         _Band(0).GridLinesBand=   1
         _Band(0).TextStyleBand=   0
   ...
   Begin MSComctlLib.ImageList ilsShG 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmShG.frx":1D73
            Key             =   ""
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmShG.frx":25C5
            Key             =   ""
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmShG.frx":4227
            Key             =   ""
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmShG.frx":4DDD
            Key             =   ""
         EndProperty
         BeginProperty ListImage5 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmShG.frx":562F
            Key             =   ""
         EndProperty
         BeginProperty ListImage6 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmShG.frx":5E81
            Key             =   ""
         EndProperty
      EndProperty
   End
...
' Resume refreshing the ado's after the gcnnConnServ connection's active transaction.
...
' Suspend refreshing the ado's due to the gcnnConnServ connection's active transaction.
...
'         Call frmMDI.SwchMnuMode(mobDatFrm.Mode) 'Switch the menu.
'         cmdOK.Enabled = mblnFrmFilld(.Index) 'Switch cmdOk.
'         If Not mblnFrmFilld(.Index) Then Call InitTxt 'Initialise.
'         If intIdxPrev = 0 Then intIdxPrev = IIf(.Index = gobSetng.FrmInvenIdx, _
'            gobSetng.FrmMatIdx, gobSetng.FrmInvenIdx) 'Initialise intIdxPrev 1st time.
'         If intIdxPrev <> .Index Then
'            Call SwchVisibleTabStop(.Index, True) 'The active TabStop's on.
...
On Error GoTo ErrHnd
   ...
   Set obSetngFrm = Me 'Suspend refreshing the ado's due to the gcnnConnServ connection's active transaction.
   ...
   With obSetngFrm
      ...
      Call Util.RefrshSetng(False)
   End With
Exit Sub
ErrHnd:
MsgBox Err.Description
Resume Next 'Resume to close gcnnConnServ's transaction.
End Sub
...
On Error GoTo ErrHnd
   ...
   gcnnConnServ.BeginTrans 'Begin transaction.
...
' REF. TO DO 1.
   ...
   If Not gobMsgInst.SQLErr(Me) Then 'Commint/rollback the transaction.
      gcnnConnServ.CommitTrans
   Else
      gcnnConnServ.RollbackTrans
   End If
Exit Sub
ErrHnd:
MsgBox Err.Description
...
'  gcnnConnServ connection's active transaction.
...
On Error GoTo ErrHnd
   With adoCtl
      If .ConnectionString <> gcnnConnServ.ConnectionString Then .ConnectionString = _
         gcnnConnServ.ConnectionString
      .Refresh
   End With
Exit Sub
ErrHnd:
...
Debug.Assert 0 = 1 'Verify .SetFocus instead of Call txtShG_GotFocus
         txtID(miShGInven).SetFocus
'         Call txtShG_GotFocus 'Focus the control.
      End If
   End If
Exit Function
ErrHnd:
...
On Error GoTo ErrHnd
   ...
   adoFld(miSexLmK).ConnectionString = gcnnConnServ.ConnectionString 'Set the data source for adoFld(miSexLmK) & dbcFld(miSexLmK).
   Call Util.DbcSetRowSrc(dbcFld(miSexLmK), adoFld(miSexLmK), "IDSex", "Sex")
   adoFld(miLiv).ConnectionString = gcnnConnServ.ConnectionString 'Set the data source for adoFld(miLiv) & dbcFld(miLiv).
...
On Error GoTo ErrHnd
    ...
    '   NoLmK.
    NoLmK = CInt(IIf(Left$(strNoLmK, 1) = "-", _
        Right$(strNoLmK, 1), strNoLmK))
Exit Function
ErrHnd:
MsgBox Err.Description
End Function
...
On Error GoTo ErrHnd
   Dim obDatTblWean As DatTbl
   Dim obDatTbl7Mon As DatTbl
   gcnnConnServ.BeginTrans 'Begin the transaction.
   With gobShGs(mstrIDFrm)
...
On Error GoTo ErrHnd
   ...
   With gobShGs(mstrIDFrm)
      ...
      If mintMode(gobSetng.FrmWtWoolIdx) = gobSetng.ModeDel Then
         obDatTbl.Del
Debug.Assert 0 = 1 'Verify .DelWtAppr with gobSetng.FrmWtWoolIdx
         If Not gobMsgInst.SQLErr(Me) Then .TblSelIdxCC.DelWtAppr gobSetng.FrmWtWoolIdx 'Delete
'         If Not gobMsgInst.SQLErr(Me) Then .TblSelIdxCC.DelWtAppr mstrFrmHostNam 'Delete
...
' Initialise the forms' ID controls & number text boxes.
Private Sub PREV_InitTxtWtWool()
On Error GoTo ErrHnd
'   With gobFormtCtl 'Simulate date masks.
'      txtNum(miWtAnnl) = .NumFrac(0)
'      txtNum(miGreaseAnnl) = .NumFrac(0)
...
On Error GoTo ErrHnd
   ...
   With deFlockBook.rsspSelect_Mate_IDDam
      If .State = adStateOpen Then .Close 'Close the recordset.
   End With
   mobUtilBrsMatLmK.SetDEParam "spSelect_Mate_IDDam", "IDDam", strIDDam 'Dam parameter.
   With flexGrid(MatLmKIdx) 'Set the grid's data source.
      Set .DataSource = deFlockBook
      .DataMember = "spSelect_Mate_IDDam"
      .Refresh
...
On Error GoTo ErrHnd
   With deFlockBook.rsspSelect_Appraisal_IDShG
      If .State = adStateOpen Then .Close 'Close the recordset.
   End With
   mobUtilBrsAppr.SetDEParam "spSelect_Appraisal_IDShG", "IDSheepGoat", strIDShG 'ShG parameter.
...
On Error GoTo ErrHnd
   ...
   With deFlockBook.rsspSelect_WeightWoolAnnl_IDShG
      If .State = adStateOpen Then .Close 'Close the recordset.
   End With
   mobUtilBrsWtWool.SetDEParam "spSelect_WeightWoolAnnl_IDShG", "IDSheepGoat", strIDShG 'ShG parameter.
