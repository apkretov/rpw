...
Private Sub InitCtl()
On Error GoTo ErrHnd
   adoOblBrs.ConnectionString = gcnnConnServ.ConnectionString 'adoOblBrs & dblOblBrs.
   With dblOblBrs
      Set .RowSource = adoOblBrs
      .BoundColumn = "IDOblast"
      .ListField = "Oblast"
   End With
   adoRayn.ConnectionString = gcnnConnServ.ConnectionString 'Set the data source for
   adoRayn.CommandType = adCmdStoredProc                    'adoRayn & dgdRayn.
   Call GridRecSrc 'Set adoRayn's record source.
   Set dgdRayn.DataSource = adoRayn 'Set data source.
   adoCountr.ConnectionString = gcnnConnServ.ConnectionString 'adoCountr & dbcCountr.
   With dbcCountr
...
On Error GoTo ErrHnd
   mobUtilDatFrm.GridRecSrc adoRayn, "spSelect_Rayons_Grid('" & mobDatFrm.IDFrm & _
      "')", dgdRayn
Exit Sub
ErrHnd:
MsgBox Err.Description
