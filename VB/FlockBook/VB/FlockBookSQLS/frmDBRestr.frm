...
Begin VB.Form frmDBRestr 
   ...
   Begin MSAdodcLib.Adodc adoDBServ 
      ...
      UserName        =   ""
      Password        =   ""
      RecordSource    =   "SELECT name FROM sysdatabases WHERE name NOT IN ('master', 'model', 'msdb', 'tempdb', 'Northwind', 'pubs')"
      Caption         =   "adoDBServ"
   ...
   Begin VB.Label lblPathSrc 
      Caption         =   "&Создать/восстановить из резервной копии:"
      Height          =   495
      Left            =   360
      TabIndex        =   5
      Top             =   2430
...
On Error GoTo ErrHnd
   ...
   With obUtilDB 'The list of the existing DB's on the server.
      adoDBServ.ConnectionString = "driver={SQL Server};server=" & .Server & _
         ";uid=" & obUtilDB.UserSA & ";pwd=" & .Pwd & ";database=" & .DBMaster
   End With
   With dblDBServ
      Set .RowSource = adoDBServ
      .BoundColumn = "name"
      .ListField = "name"
   End With
Exit Sub
...
On Error GoTo ErrHnd
   ...
   Call frmMDI.StatusMsg(True, LoadResString(104)) 'Status bar message.
   With mobUtilDB
