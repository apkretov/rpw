...
Private Sub InitCtl()
On Error GoTo ErrHnd
   adoRaynBrs.ConnectionString = gcnnConn.ConnectionString 'adoRaynBrs & dblRaynBrs.
   With dblRaynBrs
      Set .RowSource = adoRaynBrs
      .BoundColumn = "IDRayon"
      .ListField = "Rayon"
   End With
   adoTwnVll.ConnectionString = gcnnConn.ConnectionString 'Set the data source for
   adoTwnVll.CommandType = adCmdText                    'adoTwnVll & dgdTwnVll.
   Call GridRecSrc 'Set adoTwnVll's record source.
   Set dgdTwnVll.DataSource = adoTwnVll 'Set data source.
'   With adoObl 'Set the data source for adoObl & dbcObl.
      adoObl.ConnectionString = gcnnConn.ConnectionString 'adoObl & dbcObl.
...
On Error GoTo ErrHnd
   mobUtilDatFrm.GridRecSrc adoTwnVll, "SELECT TownVill.TownVill AS [Населённый пункт]" _
      & " FROM TownVill INNER JOIN Rayons ON TownVill.IDRayon = Rayons.IDRayon " & _
      "WHERE Rayons.Rayon = """ & mobDatFrm.IDFrm & """ ORDER BY TownVill.TownVill", _
      dgdTwnVll
Exit Sub
ErrHnd:
MsgBox Err.Description
