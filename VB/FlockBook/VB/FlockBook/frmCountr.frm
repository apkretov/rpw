...
Begin VB.Form frmCountr 
   ...
   Begin MSAdodcLib.Adodc adoObl 
      ...
      _Version        =   393216
   End
   Begin MSAdodcLib.Adodc adoCountrBrs 
      Height          =   330
      Left            =   240
      Top             =   4320
      Visible         =   0   'False
      ...
      _Version        =   393216
   End
   Begin MSDataListLib.DataList dblCountrBrs 
      Height          =   4155
      Left            =   240
      TabIndex        =   1
      Top             =   480
   ...
   Begin VB.TextBox txtCountr 
      ...
      Width           =   3975
   End
   Begin MSDataGridLib.DataGrid dgdObl 
      Height          =   2295
      Left            =   4920
      TabIndex        =   7
      Top             =   2280
   ...
   Begin VB.TextBox txtRemark 
      ...
      TabIndex        =   5
      Text            =   "frmCountr.frx":1042
      Top             =   1080
      Width           =   4335
   End
   Begin VB.Label lblObl 
      Caption         =   "О&бласти в стране:"
      Height          =   255
      Left            =   3120
      TabIndex        =   6
      Top             =   2280
      Width           =   1575
   End
   Begin VB.Label lblCountrBrs 
      Caption         =   "В&ыберите страну:"
      Height          =   255
      Left            =   240
      TabIndex        =   0
      Top             =   240
...
' Cancel the changes.
' Clear the controls.
' Return the focus to txtCountr
Public Sub cmdCancel_Click()
On Error GoTo ErrHnd
   mobUtilDatFrm.Cancel_Click dblCountrBrs
Exit Sub
ErrHnd:
...
On Error GoTo ErrHnd
   mobUtilDatFrm.Ok_Click dblCountrBrs, LoadResString(293)
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub
...
On Error GoTo ErrHnd
   With mobTblCountr
      .Countr = Trim$(txtCountr)
'      .IDCountr = CLng(dbcCountr.BoundText)
...
On Error GoTo ErrHnd
   ...
   If Not blnPassed Then 'Execute once.
      blnPassed = True
      Set mobDatFrm = Me
      Set mobTblCountr = New TblCountr
      Set obDatTbl = mobTblCountr
...
' Shwitch on/off the data form's controls depending on whether the txtCountr is filled.
Private Sub DatFrm_SwchCtlFill(ByVal blnEnable As Boolean, Optional ByVal blnEnableID _
   As Boolean)
On Error GoTo ErrHnd
   mobUtilDatFrm.DatFrm_SwchCtlFill blnEnable, blnEnableID, lblCountr.Name, _
      lblCountrBrs.Name, dblCountrBrs.Name
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub
...
On Error GoTo ErrHnd
   With mobUtilDatFrm
      .DatFrm_SwchCtlMode lblCountrBrs.Name, dblCountrBrs.Name, txtRemark.Name
      .SwchGridMode dgdObl 'Disable dgdFarm in the Add mode.
   End With
Exit Sub
ErrHnd:
MsgBox Err.Description
...
' Syncronise the txtCountr with this control.
Private Sub dblCountrBrs_Click()
On Error GoTo ErrHnd
   txtCountr = dblCountrBrs
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub
...
' Press Enter.
Private Sub dblCountrBrs_DblClick()
On Error GoTo ErrHnd
...
' Call the txtCountr_KeyPress procedure in the Edit/Del mode, if Enter is pressed.
Private Sub dblCountrBrs_KeyPress(KeyAscii As Integer)
On Error GoTo ErrHnd
...
On Error GoTo ErrHnd
   ...
   txtCountr = gobDatFrms.IDFrmLast(Me.Name) 'Set the last ID.
...
On Error GoTo ErrHnd
   mobUtilDatFrm.Form_Resize dblCountrBrs, dgdObl.Name
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub
...
' Write the controls' content to the the mobTblCountr object.
Private Sub DatFrm_FrmToDB()
On Error GoTo ErrHnd
   mobUtilDatFrm.DatFrm_FrmToDB LoadResString(295), LoadResString(319)
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub
...
' Press Enter.
Private Sub txtCountr_DblClick()
On Error GoTo ErrHnd
...
' Format the control.
Private Sub txtCountr_GotFocus()
On Error GoTo ErrHnd
   gobFormtCtl.SelTxt txtCountr
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub

' Call mobDatFrm.IDCtlPress procedure.
Private Sub txtCountr_KeyPress(KeyAscii As Integer)
On Error GoTo ErrHnd
...
Private Sub txtCountr_LostFocus()
On Error GoTo ErrHnd
...
Private Sub InitCtl()
On Error GoTo ErrHnd
   adoCountrBrs.ConnectionString = gcnnConn.ConnectionString 'adoCountrBrs &
   With dblCountrBrs                                             'dblCountrBrs.
      Set .RowSource = adoCountrBrs
      .BoundColumn = "IDCountry"
      .ListField = "Country"
   End With
   adoObl.ConnectionString = gcnnConn.ConnectionString 'adoObl & dgdObl.
   Call GridRecSrc 'Set adoObl's record source.
   Set dgdObl.DataSource = adoObl 'Set data source.
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub

' Set the adoObl control's record source.
' Resize the dgdObl grid's columns.
Private Sub GridRecSrc()
On Error GoTo ErrHnd
   mobUtilDatFrm.GridRecSrc adoObl, "SELECT Oblasts.Oblast AS Область FROM Countries " & _
       "INNER JOIN Oblasts ON Countries.IDCountry = Oblasts.IDCountry WHERE " & _
       "Countries.Country = """ & mobDatFrm.IDFrm & """ ORDER BY Oblasts.Oblast", dgdObl
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub
...
Private Sub txtCountr_Validate(Cancel As Boolean)
On Error GoTo ErrHnd
   mobUtilDatFrm.IDCtl_Validate Cancel, lblCountr
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub
