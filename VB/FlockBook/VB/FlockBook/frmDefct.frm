...
Begin VB.Form frmDefct 
   ...
   Begin MSAdodcLib.Adodc adoShG 
      ...
      _Version        =   393216
   End
   Begin MSAdodcLib.Adodc adoDefctBrs 
      Height          =   330
      Left            =   240
      Top             =   4320
      Visible         =   0   'False
   ...
   Begin VB.TextBox txtRemark 
      ...
      Width           =   3975
   End
   Begin MSDataListLib.DataList dblDefctBrs 
      Height          =   4155
      Left            =   240
      TabIndex        =   1
      Top             =   480
   ...
   Begin VB.Label lblShG 
      ...
      Width           =   1095
   End
   Begin VB.Label lblDefctBrs 
      Caption         =   "В&ыберите порок:"
      Height          =   255
      Left            =   240
      TabIndex        =   0
      Top             =   240
...
' Cancel the changes.
' Clear the controls.
' Return the focus to txtDefct
Public Sub cmdCancel_Click()
On Error GoTo ErrHnd
   mobUtilDatFrm.Cancel_Click dblDefctBrs
Exit Sub
ErrHnd:
...
On Error GoTo ErrHnd
   mobUtilDatFrm.Ok_Click dblDefctBrs, LoadResString(284)
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub
...
On Error GoTo ErrHnd
   With mobTblDefct
      txtRemark = .Remark
   End With
   Call GridRecSrc 'Set adoShG's record source.
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub
...
On Error GoTo ErrHnd
   ...
   If Not blnPassed Then 'Execute once.
      blnPassed = True
      Set mobDatFrm = Me
      Set mobTblDefct = New TblDefct
      Set obDatTbl = mobTblDefct
...
' Shwitch on/off the data form's controls depending on whether the txtDefct is filled.
Private Sub DatFrm_SwchCtlFill(ByVal blnEnable As Boolean, Optional ByVal blnEnableID _
   As Boolean)
On Error GoTo ErrHnd
   mobUtilDatFrm.DatFrm_SwchCtlFill blnEnable, blnEnableID, lblDefct.Name, _
      lblDefctBrs.Name, dblDefctBrs.Name
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub
...
On Error GoTo ErrHnd
   With mobUtilDatFrm
      .DatFrm_SwchCtlMode lblDefctBrs.Name, dblDefctBrs.Name, txtRemark.Name
      .SwchGridMode dgdShG 'Disable dgdFarm in the Add mode.
   End With
Exit Sub
ErrHnd:
MsgBox Err.Description
...
' Syncronise the txtDefct with this control.
Private Sub dblDefctBrs_Click()
On Error GoTo ErrHnd
   txtDefct = dblDefctBrs
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub
...
Private Sub dblDefctBrs_DblClick()
On Error GoTo ErrHnd
...
' Call the txtDefct_KeyPress procedure in the Edit/Del mode, if Enter is pressed.
Private Sub dblDefctBrs_KeyPress(KeyAscii As Integer)
On Error GoTo ErrHnd
...
On Error GoTo ErrHnd
   ...
   txtDefct = gobDatFrms.IDFrmLast(Me.Name) 'Set the last ID.
...
On Error GoTo ErrHnd
   mobUtilDatFrm.Form_Resize dblDefctBrs, dgdShG.Name
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub
...
On Error GoTo ErrHnd
   ...
   Call GridRecSrc 'Reset adoShG's record source.
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub

' Write the controls' content to the the mobTblDefct object.
...
' Format the control.
Private Sub txtDefct_GotFocus()
On Error GoTo ErrHnd
   gobFormtCtl.SelTxt txtDefct
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub
...
Private Sub txtDefct_KeyPress(KeyAscii As Integer)
On Error GoTo ErrHnd
...
Private Sub txtDefct_LostFocus()
On Error GoTo ErrHnd
...
Private Sub InitCtl()
On Error GoTo ErrHnd
   adoDefctBrs.ConnectionString = gcnnConn.ConnectionString 'adoDefctBrs & dblDefctBrs.
   With dblDefctBrs
      Set .RowSource = adoDefctBrs
      .BoundColumn = "IDDefect"
      .ListField = "Defect"
   End With
   adoShG.ConnectionString = gcnnConn.ConnectionString 'Set the data source for
'   adoShG.CommandType = adCmdStoredProc                    'adoShG & dgdShG.
   Call GridRecSrc 'Set adoShG's record source.
...
' Set the adoShG control's record source.
Private Sub GridRecSrc()
On Error GoTo ErrHnd
   mobUtilDatFrm.GridRecSrc adoShG, "SELECT Appraisal.IDSheepGoat AS Номер, " & _
      "Appraisal.Age AS Цикл, Appraisal.DateAppr AS Дата FROM Defects INNER JOIN " & _
      "(Appraisal INNER JOIN InhertDefect ON (Appraisal.Age = InhertDefect.Age) AND " & _
      "(Appraisal.IDSheepGoat = InhertDefect.IDSheepGoat)) ON Defects.IDDefect = " & _
      "InhertDefect.IDDefect WHERE Defects.Defect = """ & mobDatFrm.IDFrm & """", dgdShG
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub
...
Private Sub txtDefct_Validate(Cancel As Boolean)
On Error GoTo ErrHnd
   mobUtilDatFrm.IDCtl_Validate Cancel, lblDefct
Exit Sub
ErrHnd:
MsgBox Err.Description
End Sub
