...
' Execute the cmdOk_Click procedure's part 2 after querying the data.
...
' Get the interval for the frmMDI form's timer based on the report index.
Private Property Get IdxTimeIntrv() As Long
On Error GoTo ErrHnd
   Const lngSumGenEval As Long = 4000 'Timer intervals.
   Const lngSumPerform As Long = 1000
   Const lngFrmInvMat As Long = 3000
   Const lngFrmLmK As Long = 1000
   Const lngFrmWtLmK As Long = 1000
   Const lngFrmWtWool As Long = 1000
   Const lngFrmAppr As Long = 1000
   Select Case mintIdx
   Case mintIdxSumGenEval 'rptSumGenEval
      IdxTimeIntrv = lngSumGenEval
   Case mintIdxSumPerform 'rptSumPerform
      IdxTimeIntrv = lngSumPerform
   Case mintIdxFrmInvMat 'rptFrmInvMat
      IdxTimeIntrv = lngFrmInvMat
   Case mintIdxFrmLmK 'rptFrmInvMat
      IdxTimeIntrv = lngFrmLmK
   Case mintIdxFrmWtLmK 'rptFrmWtLmK
      IdxTimeIntrv = lngFrmWtLmK
   Case mintIdxFrmWtWool 'rptFrmWtWool
      IdxTimeIntrv = lngFrmWtWool
   Case mintIdxFrmAppr 'rptFrmAppr
      IdxTimeIntrv = lngFrmAppr
   End Select
Exit Property
ErrHnd:
MsgBox Err.Description
