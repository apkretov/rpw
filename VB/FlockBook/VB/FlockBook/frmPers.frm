...
Private Sub InitCtl()
On Error GoTo ErrHnd
   adoNamBrs.ConnectionString = gcnnConn.ConnectionString 'adoNamBrs & dblNamBrs.
   With dblNamBrs
      Set .RowSource = adoNamBrs
      .BoundColumn = "IDPers"
      .ListField = "Nam"
   End With
   adoFarmFarmer.ConnectionString = gcnnConn.ConnectionString
   adoFarmCont.ConnectionString = gcnnConn.ConnectionString
...
On Error GoTo ErrHnd
   With mobUtilDatFrm
      .GridRecSrc adoFarmFarmer, "SELECT Farms.IDFarm AS [Фермер на ферме] FROM " & _
         "Persons INNER JOIN Farms ON Persons.IDPers = Farms.IDPersFarmer WHERE " & _
         "Persons.Nam = """ & mobDatFrm.IDFrm & """ ORDER BY Persons.Nam", dgdFarmFarmer
      .GridRecSrc adoFarmCont, "SELECT Farms.IDFarm AS [Контактное лицо на ферме] " & _
         "FROM Persons INNER JOIN Farms Farms ON Persons.IDPers = Farms.IDPersCont " & _
         "WHERE Persons.Nam = """ & mobDatFrm.IDFrm & """ ORDER BY Persons.Nam", _
         dgdFarmCont
   End With
Exit Sub
ErrHnd:
