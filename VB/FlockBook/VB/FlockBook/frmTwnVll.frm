...
Private Sub InitCtl()
On Error GoTo ErrHnd
   adoTwnVllBrs.ConnectionString = gcnnConn.ConnectionString 'adoTwnVllBrs & dblTwnVllBrs.
   With dblTwnVllBrs
      Set .RowSource = adoTwnVllBrs
      .BoundColumn = "IDTownVill"
      .ListField = "TownVill"
   End With
   adoRayn.ConnectionString = gcnnConn.ConnectionString 'adoRayn & dbcRayn.
   With dbcRayn
      Set .RowSource = adoRayn
      .BoundColumn = "IDRayon"
      .ListField = "Rayon"
   End With
   adoPers.ConnectionString = gcnnConn.ConnectionString 'adoPers & dgdPers.
   adoFarm.ConnectionString = gcnnConn.ConnectionString 'adoFarm & dgdFarm.
...
On Error GoTo ErrHnd
   With mobUtilDatFrm
      .GridRecSrc adoPers, "SELECT Persons.Nam AS [Участник] FROM TownVill INNER JOIN " & _
         "Persons ON TownVill.IDTownVill = Persons.IDTownVill WHERE TownVill.TownVill " _
         & "= """ & mobDatFrm.IDFrm & """ ORDER BY Persons.Nam", dgdPers
      .GridRecSrc adoFarm, "SELECT Farms.IDFarm AS Ферма FROM TownVill INNER JOIN " & _
         "Farms ON TownVill.IDTownVill = Farms.IDTownVill WHERE TownVill.TownVill = """ _
         & mobDatFrm.IDFrm & """ ORDER BY Farms.IDFarm", dgdFarm
   End With
Exit Sub
ErrHnd:
MsgBox Err.Description
